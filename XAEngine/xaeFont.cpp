...
xaeFont::xaeFont(const char* lpsFontName, int nFaceSize, BOOL bBold, BOOL bItalic, BOOL bAntialias)
{
...
// 渲染文本
void xaeFont::Print( float x, float y, const char *format, ... )
{
...
// 渲染文本
void xaeFont::Printf( float x, float y, const wchar_t *Str)
{
	Render(x,y,Str); 
}

void xaeFont::Render(float x, float y, const wchar_t* text )
{
	float offsetX = x;
	float offsetY = y;
...
// 设置与获取颜色
void xaeFont::SetColor( DWORD dwColor, int i )
{
	m_pSprite->SetColor(dwColor,i);
}

DWORD xaeFont::GetColor(int i)
{
	return m_pSprite->GetColor(i);
}

// 获取文本宽高
SIZE xaeFont::GetTextSize( const wchar_t* text )
{
...
// 根据坐标获取字符
wchar_t xaeFont::GetCharacterFromPos( const wchar_t* text, float pixel_x, float pixel_y )
{
	float x = 0;
	float y = 0;
...
// 设置字间距
void xaeFont::SetKerningWidth( float kerning )
{
	m_nKerningWidth = kerning;
}
void xaeFont::SetKerningHeight( float kerning )
{
	m_nKerningHeight = kerning;
}

// 获取字间距
float xaeFont::GetKerningWidth()
{
	return m_nKerningWidth;
}
float xaeFont::GetKerningHeight()
{
	return m_nKerningHeight;
}	

// 字体大小
float xaeFont::GetFontSize()
{
	return m_nFontSize;
}

// 根据字符获取轮廓
unsigned int xaeFont::GetGlyphByCharacter( wchar_t c )
{
	unsigned int idx = (unsigned int)c;
	if (NULL == (m_Glyphs[idx].t)) CacheCharacter(idx,c);
	return idx;
}
inline float xaeFont::GetWidthFromCharacter( wchar_t c, bool original )
{
...
inline void xaeFont::CacheCharacter(unsigned int idx, wchar_t c)
{
